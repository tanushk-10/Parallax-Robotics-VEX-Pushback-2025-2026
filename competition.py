...
#                                                                              #
#   Module:       main.py                                                      #
#   Description:  V5 Competition Main - Driver Control + Scoring Autonomous    #
#                                                                              #
# ---------------------------------------------------------------------------- #
...
if DRIVE_OFF_WALL_IN is None:
    if abs(START_ANGLE_DEG) < 1.0:
        ...
        DRIVE_OFF_WALL_IN = max(
            0.0,
            math.sqrt(2.0) * _HALF - START_OUT_IN
        ) + 8.0
...
TURN_TO_GOAL_DEG = math.degrees(
    math.atan2(_v_lat, _v_out) + math.pi - _A
)
...
def lift_position():
    return (
        lift_left.position(DEGREES)
        + lift_right.position(DEGREES)
    ) / 2.0
...
def lift_check():
    ...
    brain.screen.print("Check cable and port")
...
def lift_telemetry(mode, pos):
    ...
    if mode == "UP" and lift_volts >= STALL_ARM_VOLTS:
        ...
        if la < 0 or ra < 0:
            ...
            max(la, ra) > 0.6
            and min(la, ra) < 0.25 * max(la, ra)
        ):
            side = "LEFT" if la > ra else "RIGHT"
            lift_verdict = "ONLY " + side + " PULLS - other idle"
    ...
    message = lift_verdict if lift_verdict else "hold L1 to test"

    brain.screen.print("> {:<30}".format(message))
...
def lift_control():
    ...
    temp = max(
        lift_left.temperature(TemperatureUnits.CELSIUS),
    ...
    if mode == "UP":
        ...
        lift_volts = min(
            UP_VOLTS,
            lift_volts + SLEW_VOLTS_PER_LOOP
        )
    ...
    if mode == "UP":
        watching = (
            STALL_GUARD
            and lift_volts >= STALL_ARM_VOLTS
        )
...
def claw_screen():
    wrist_text = "UP  " if pivot_up else "DOWN"
    grip_text = "HOLD" if grab_closed else "OPEN"

    controller.screen.set_cursor(2, 1)
    controller.screen.print(
        "Wrist " + wrist_text + " Grip " + grip_text
    )
...
def wheel_speed_in_s():
    rpm = (
        abs(left_drive.velocity(RPM))
        + abs(right_drive.velocity(RPM))
    ) / 2.0
...
def turn_degrees(angle, speed=40, timeout_ms=None):
    ...
    last_turn = "turn ask {:.0f} enc {:.0f} {:<7}".format(
        ...
        "TIMEOUT" if elapsed >= timeout_ms else "ok"
    )
...
def pre_autonomous():
    ...
    brain.screen.clear_screen()
    brain.screen.set_cursor(1, 1)
    brain.screen.print("COMPETITION READY")
...
def autonomous():
    ...
    controller.screen.clear_screen()
    controller.screen.set_cursor(1, 1)
    controller.screen.print("AUTON STARTED")
    ...
    if abs(DRIVE_TO_GOAL_IN) > 0.1:
        step(
            "reverse {:.1f}in".format(
                abs(DRIVE_TO_GOAL_IN)
            )
        )
...
while True:
    ...
    if comp.is_autonomous():
        brain.screen.print("MODE: AUTON       ")
    elif comp.is_driver_control():
        brain.screen.print("MODE: DRIVER      ")
    else:
        brain.screen.print("MODE: DISABLED    ")
